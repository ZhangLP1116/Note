...
if __name__ == '__main__':
    ...
    config = r'E:\yuan\g\config.ini'
    plug = r'E:\yuan\g\PCGameSDK.dll'
    ...
    if value == "b":
        print("当前注册表为b服，进行替换操作")
        os.system(r'del "%s\\config.ini"' % config_path)
        os.system(r'del "%s\\PCGameSDK.dll"' % plug_path)
        os.system(r'copy %s "%s"' % (config, config_path))
        # 导出B服注册表项
        os.system(r'del "%s"' % b_reg_path)
        os.system(r'REG  EXPORT HKEY_CURRENT_USER\SOFTWARE\miHoYo\原神  %s' % b_reg_path)
        # 导入官服注册表项
        os.system(r'REG IMPORT  %s' % g_reg_path)
        winreg.SetValueEx(key, "miHoYo", 0, key_type, "g")
        print("操作完成")
    else:
        print("当前注册表已经为官服，不需要替换操作")
    # os.system(r'"%s"' % exe_path)
    os.chdir(work_path)
